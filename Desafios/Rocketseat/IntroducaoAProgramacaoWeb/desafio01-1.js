...
Comece criando constantes para armazenar nome, sexo, idade e contribuicao
(em anos), por exemplo:
...
O tempo de contribuição mínimo para homens é de 35 anos e, para mulheres, 30 
anos;
...
if(sexo === 'F'){
  if((contribuicao >=30) && (idade + contribuicao >= 85)){
    ...
    console.log(`Sra. ${nome}, você não pode se aposentar`)
  }
} else {
    if((contribuicao >= 35) && (idade + contribuicao >= 95)){
      console.log(`Sr. ${nome}, você pode se aposentar`)
    }else{
      console.log(`Sr. ${nome}, você não pode se aposentar`)
  }
}